...
void SmartMAPFPlanner::plan(int time_limit, vector<ActionType> &actions) {
    ...
    //     {
    //         actions[i] = ActionType::FORWARD; //forward action
    //     }
    ...
    //         if (incr == 1 || incr == -3)
    //         {
    //             actions[i] = ActionType::ROTATE; //C--counter clockwise rotate
    //         }
    //         else if (incr == -1 || incr == 3)
    //         {
    //             actions[i] = ActionType::COUNTER_ROTATE; //CCR--clockwise rotate
    //         }
    //     }
...
// plan using simple A* that ignores the time dimension
void MAPFPlanner::plan(int time_limit, vector<ActionType> &actions) {
    // NOTE we need to return within time_limit, but we can exploit this time duration as much as possible
