            ...
            for (int timestep = start_time; timestep < size1; timestep++) {
                if (size2 <= timestep - k_robust)
                    ...
                    if (loc1 == loc2) {
                        conflicts.emplace_back(new Conflict(a1, a2, loc1, -1, timestep));
                        return;
                    } else if (timestep < size1 - 1 && timestep < size2 - 1 && loc1 == paths[a2]->at(timestep + 1).location && loc2 == paths[a1]->at(timestep + 1).location) {
                        conflicts.emplace_back(new Conflict(a1, a2, loc1, loc2, timestep + 1));// edge conflict
                        return;
                    }
                }
    ...
    /*double ECBS::get_path_cost(const Path& path) const
{
    ...
    bool ECBS::run(const std::vector<State> &starts,
        ...
        while (!open_list.empty() && !solution_found) {
            ...
            if (screen == 2)
                std::cout << "Expand Node " << curr->time_generated << " ( cost = " << curr->f_val << " , min_cost = " << curr->min_f_val << ", #conflicts = " << curr->num_of_collisions << " ) on conflict " << *curr->conflict << std::endl;
            ECBSNode *n[2];
            for (int i = 0; i < 2; i++)
    ...
    ECBS::ECBS(const BasicGraph &G, SingleAgentSolver &path_planner) : MAPFSolver(G, path_planner), disjoint_splitting(false) {}
        ...
        for (int a1 = 0; a1 < num_of_agents; a1++) {
            for (int a2 = a1 + 1; a2 < num_of_agents; a2++) {
                ...
                if (!conflict.empty()) {
                    ...
                    if (loc2 < 0)
                        std::cout << "Agents " << a1 << " and " << a2 << " collide at " << loc1 << " at timestep " << t << std::endl;
                    else
                        std::cout << "Agents " << a1 << " and " << a2 << " collide at (" << loc1 << "-->" << loc2 << ") at timestep " << t << std::endl;
                    return false;
                }
            }
        }
    ...
    void ECBS::print_results() const {
        ...
        std::cout << runtime << "," << HL_num_expanded << "," << HL_num_generated << "," << LL_num_expanded << "," << LL_num_generated << "," << solution_cost << "," << min_f_val << "," << avg_path_length << "," << dummy_start->num_of_collisions << "," << window << std::endl;
    }

    void ECBS::save_results(const std::string &fileName, const std::string &instanceName) const {
        std::ofstream stats;
        stats.open(fileName, std::ios::app);
        stats << runtime << "," << HL_num_expanded << "," << HL_num_generated << "," << LL_num_expanded << "," << LL_num_generated << "," << solution_cost << "," << min_f_val << "," << avg_path_length << "," << dummy_start->num_of_collisions << "," << instanceName << "," << window << std::endl;
        stats.close();
    }
