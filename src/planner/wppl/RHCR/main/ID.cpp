...
namespace RHCR {
    ...
    bool ID::has_conflicts(const Path &path1, const Path &path2) const {
        ...
        if (solver.hold_endpoints) {
            ...
            for (size_t timestep = 0; timestep < min_path_length; timestep++) {
                ...
                if (loc1 == loc2) {
                    return true;
                } else if (timestep < min_path_length - 1 && loc1 == path2[timestep + 1].location && loc2 == path1[timestep + 1].location) {
                    return true;
                }
            }
            ...
            for (int timestep = 0; timestep < size1; timestep++) {
                if (size2 <= timestep - k_robust)
                    ...
                    if (loc1 == loc2) {
                        return true;
                    } else if (timestep < size1 - 1 && timestep < size2 - 1 && loc1 == path2[timestep + 1].location && loc2 == path1[timestep + 1].location) {
                        return true;
                    }
                }
            }
    ...
    void ID::save_results(const string &fileName, const string &instanceName) const {
        ...
        stats << runtime << "," << num_of_groups << "," << largest_group << "," << 0 << "," << 0 << "," << 0 << "," << 0 << "," << 0 << "," << 0 << "," << instanceName << std::endl;
        stats.close();
    }
