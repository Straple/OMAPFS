    ...
    void LRAStar::resolve_conflicts(const vector<Path> &input_paths) {
        ...
        for (int t = 1; t <= simulation_window; t++) {
            ...
            for (auto agent: agents_list) {
                ...
                if (loc == solution[agent][t - 1].location) {// The agent wait or rotates at its current location
                ...
                } else if (curr_locations.find(loc) != curr_locations.end()) {// The agent cannot move because its next location is occupied currently
                ...
                } else// The agent wants to move to a location that is currently empty
                {
                    ...
                    if (other == next_locations.end()) {// No other agents want to go to this location yet
                        solution[agent].emplace_back(loc, t, orientation);
                        path_pointers[agent]++;
                    } else {// Another agent also wants to go to this location, for now we just force this agent to wait
                        wait_command(agent, t, path_pointers);
                    }
                }
