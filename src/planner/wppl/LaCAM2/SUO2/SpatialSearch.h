...
#include <planner/wppl/LaCAM2/SUO2/SpatialState.h>
...
namespace SUO2 {
    ...
    namespace Spatial {
        ...
        public:
            ...
            AStar(
                    const Environment &env, const std::vector<float> &weights, std::shared_ptr<HeuristicTable> &HT, int planning_window) : env(env), weights(weights), cost_map(env.cols, env.rows), HT(HT), planning_window(planning_window){
