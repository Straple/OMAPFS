...
namespace LNS {
    ...
    namespace Parallel {
        ...
        NeighborGenerator::NeighborGenerator(
                ...
                int num_threads, bool fix_ng_bug, int screen, int random_seed) : instance(instance), HT(HT), path_table(path_table),
                ...
                if (neighbor.old_sum_of_costs > neighbor.sum_of_costs) {
                    if (fix_ng_bug) {
                        destroy_weights[neighbor.selected_neighbor] =
                                reaction_factor * (neighbor.old_sum_of_costs - neighbor.sum_of_costs) / neighbor.agents.size() + (1 - reaction_factor) * destroy_weights[neighbor.selected_neighbor];
                    } else {
                        destroy_weights[neighbor.selected_neighbor] =
                                reaction_factor * std::max((neighbor.old_sum_of_costs - neighbor.sum_of_costs) / neighbor.agents.size(), 1.0f) + (1 - reaction_factor) * destroy_weights[neighbor.selected_neighbor];
                    }
                } else
                    destroy_weights[neighbor.selected_neighbor] =
