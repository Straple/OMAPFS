...
namespace LNS {
    ...
    namespace Parallel {
        ...
        enum destroy_heuristic { RANDOMAGENTS,
                                 RANDOMWALK,
                                 INTERSECTION,
                                 DESTORY_COUNT };
        ...
        struct Agent {
            ...
            Agent(int id, const Instance &instance, std::shared_ptr<HeuristicTable> &HT, std::shared_ptr<std::vector<LaCAM2::AgentInfo>> &agent_infos) : id(id), HT(HT), instance(instance), agent_infos(agent_infos) {
            }

            Agent(const Agent &agent) : id(agent.id), HT(agent.HT), instance(agent.instance), agent_infos(agent.agent_infos), path(agent.path) {
            }
