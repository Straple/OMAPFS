#include <planner/wppl/LNS/Parallel/LocalOptimizer.h>
...
namespace LNS {
    ...
    namespace Parallel {
        ...
        LocalOptimizer::LocalOptimizer(
                ...
                int screen,
                int random_seed) : instance(instance), path_table(instance.map_size, window_size_for_CT), HT(HT), map_weights(map_weights), agent_infos(agent_infos),
        ...
        bool LocalOptimizer::runPP(Neighbor &neighbor, const TimeLimiter &time_limiter) {
            ...
            while (p != shuffled_agents.end()) {
                ...
                if (screen >= 3)
                    cout << "Remaining agents = " << remaining_agents << ", remaining time = " << time_limiter.get_remaining_time() << " seconds. " << endl
