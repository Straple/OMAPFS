...
namespace LNS {
    ...
    bool Instance::validateSolution(const vector<Path *> &paths, int sum_of_costs, int num_of_colliding_pairs) const {
        ...
        if (sum_of_costs != sum) {
            cout << "The computed sum of costs " << sum_of_costs << " is different from that of the solution " << sum << endl;
            exit(-1);
        }
        // check for colliions
        int collisions = 0;
            ...
            for (auto j = i + 1; j < start_locations.size(); j++) {
                ...
                for (; t < (int) paths[a1]->size(); t++) {
                    if (paths[a1]->at(t).location == paths[a2]->at(t).location)// vertex conflict
                    {
                        if (num_of_colliding_pairs == 0) {
                            cout << "Find a vertex conflict between agents " << a1 << " and " << a2 << " at location " << paths[a1]->at(t).location << " at timestep " << t << endl;
                            exit(-1);
                        }
                        collisions++;
                    ...
                    {
                        if (num_of_colliding_pairs == 0) {
                            cout << "Find an edge conflict between agents " << a1 << " and " << a2 << " at edge (" << paths[a1]->at(t - 1).location << "," << paths[a1]->at(t).location << ") at timestep " << t << endl;
                            exit(-1);
                        }
                        collisions++;
                        found_collision = true;
                ...
                if (!found_collision) {
                    ...
                    for (; t < (int) paths[a2]->size(); t++) {
                        ...
                        {
                            if (num_of_colliding_pairs == 0) {
                                cout << "Find a target conflict where agent " << a2 << " (of length " << paths[a2]->size() - 1 << ") traverses agent " << a1 << " (of length " << paths[a1]->size() - 1 << ")'s target location " << target << " at timestep " << t << endl;
                                exit(-1);
                            }
                            collisions++;
                            break;
        ...
        if (collisions != num_of_colliding_pairs) {
            cout << "The computed number of colliding pairs " << num_of_colliding_pairs << " is different from that of the solution " << collisions << endl;
            exit(-1);
        }
        cout << "Done!" << endl;
        return true;
