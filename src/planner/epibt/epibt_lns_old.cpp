...
bool EPIBT_LNS_OLD::consider() {
    return old_score - 1e-6 <= cur_score
           // old_score > cur_score
           || rnd.get_d() < std::exp(-((old_score - cur_score) / old_score) / temp);
}

bool EPIBT_LNS_OLD::validate_path(uint32_t r, uint32_t desired) const {
...
bool EPIBT_LNS_OLD::is_free_path(uint32_t r) const {
    ASSERT(0 <= r && r < robots.size(), "invalid r");
...
Path EPIBT_LNS_OLD::get_path(uint32_t r, uint32_t desired) const {
    ASSERT(0 <= r && r < robots.size(), "invalid r");
    ASSERT(0 <= desired && desired < get_operations().size(), "invalid desired");
...
uint32_t EPIBT_LNS_OLD::get_used(uint32_t r) const {
    uint32_t answer = -1;
...
std::vector<uint32_t> EPIBT_LNS_OLD::get_multi_used(uint32_t r) const {
...
int64_t EPIBT_LNS_OLD::get_smart_dist_IMPL(uint32_t r, uint32_t desired) const {
    ...
    if (op.back() == ActionType::WAIT) {
        ...
        if (op[op.size() - 2] == ActionType::WAIT) {
            uint32_t to = node;
...
int64_t EPIBT_LNS_OLD::get_smart_dist(uint32_t r, uint32_t desired) const {
    return smart_dist_dp[r][desired];
}

void EPIBT_LNS_OLD::update_score(uint32_t r, uint32_t desired, double &cur_score, int sign) const {
    int32_t diff = get_smart_dist(r, 0) - get_smart_dist(r, desired);
    cur_score += sign * diff * robot_power[r];
}

void EPIBT_LNS_OLD::add_path(uint32_t r) {
    ASSERT(0 <= r && r < robots.size(), "invalid r");
    ASSERT(0 <= desires[r] && desires[r] < get_operations().size(), "invalid desired");
...
void EPIBT_LNS_OLD::remove_path(uint32_t r) {
    ASSERT(0 <= r && r < robots.size(), "invalid r");
    ASSERT(0 <= desires[r] && desires[r] < get_operations().size(), "invalid desired");
...
uint32_t EPIBT_LNS_OLD::try_echo_slam(std::vector<uint32_t> &rids, uint32_t &counter, uint32_t depth) {
    if (rids.empty()) {
        if (consider()) {
            return 1;// accepted
        } else {
...
bool EPIBT_LNS_OLD::try_echo_slam(uint32_t r) {
    ++visited_counter;
    old_score = cur_score;
    remove_path(r);
    uint32_t counter = 0;
...
uint32_t EPIBT_LNS_OLD::try_build(uint32_t r, uint32_t &counter, uint32_t depth) {
    if (counter > 1000 ||//
        (counter % 16 == 0 && get_now() >= end_time)) {
        counter = -1;
        return 2;
...
bool EPIBT_LNS_OLD::try_build(uint32_t r) {
    ++visited_counter;
    old_score = cur_score;
    remove_path(r);
    uint32_t counter = 0;
...
/*uint32_t
EPIBT_LNS_OLD::try_rebuild_neighbors(uint32_t id, const std::vector<uint32_t> &rids, uint32_t &counter, uint32_t depth) {
    if (id == rids.size()) {
        // все поставили
        if (consider()) {
            return 1;// accepted
...
bool EPIBT_LNS_OLD::try_rebuild_neighbors(uint32_t r) {
    ++visited_counter;
    old_score = cur_score;
...
uint32_t EPIBT_LNS_OLD::build(uint32_t r, uint32_t depth, uint32_t &counter) {
    if (counter == -1 ||//
        //counter > 30'000 ||//
        (counter % 16 == 0 && get_now() >= end_time)) {
        counter = -1;
...
bool EPIBT_LNS_OLD::build(uint32_t r) {
    ++visited_counter;
    old_score = cur_score;
    remove_path(r);
    uint32_t counter = 0;
...
void EPIBT_LNS_OLD::reset(uint32_t r, std::vector<uint32_t> &destroyed) {
...
EPIBT_LNS_OLD::EPIBT_LNS_OLD(Robots &robots, TimePoint end_time)
...
void EPIBT_LNS_OLD::solve(uint64_t seed) {
    ...
    for (uint32_t step = 0; get_now() < end_time; step++) {
...
std::vector<ActionType> EPIBT_LNS_OLD::get_actions() const {
    ...
    for (uint32_t r = 0; r < robots.size(); r++) {
        ...
        if (best_desires[r] == 0) {
            ...
            if (dist == get_hm().get(get_graph().get_to_node(robots[r].node, 1), robots[r].target)) {
                answer[r] = ActionType::ROTATE;
            } else if (dist == get_hm().get(get_graph().get_to_node(robots[r].node, 2), robots[r].target)) {
                answer[r] = ActionType::COUNTER_ROTATE;
            } else {
                answer[r] = ActionType::WAIT;
            }
        }
    }
    return answer;
